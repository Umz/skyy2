...
/** Always stay close to target but mix feints into the fray */
export default class Bandit4 extends ActionManager {
  ...
  hitAndRun(target) {
    ...
    const offX = startX > target.x ? -40 : 40;
  ...
  //  Defend if hit
  defend() {
    const target = GetClosestEnemyWithinRange(this.sprite, 100);
    this.addActions(
      new ListenState(this.sprite, Enum.SS_READY),
      new ActComplete(()=>this.sprite.faceX(target.x)),
      new ListenCondition(()=> this.sprite.isFacing(target.x), 350),
      new ActDefend(this.sprite, 1000)
    );
  }
